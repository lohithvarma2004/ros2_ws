...
void FinROSPlugin::Load(gazebo::physics::ModelPtr _parent, sdf::ElementPtr _sdf)
{
  ...
  if (!rclcpp::ok())
  {
    gzerr << "Not loading plugin since ROS has not been properly initialized.  Try starting gazebo with ros plugin:\n"
          << "  gazebo -s libgazebo_ros_api_plugin.so\n";
    return;
  }

  myRosNode = gazebo_ros::Node::CreateWithArgs(_sdf->Get<std::string>("name"));
  
  // FIX: Use a format string and pass the namespace as an argument
  ...
  myPubFinForce = myRosNode->create_publisher<geometry_msgs::msg::WrenchStamped>(wrenchTopic, 10);
